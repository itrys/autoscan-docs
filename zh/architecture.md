...
4. 应用排除过滤器（v1.1.0+）
5. 设置自定义注解过滤器（v1.1.0+）
6. 创建 `ClassPathBeanDefinitionScanner` 进行扫描
7. 注册扫描到的组件到 Spring 容器
8. 输出扫描日志（开发模式下）
...
2. **解析通配符**（v1.1.0+）- 将包路径中的通配符模式解析为实际的包路径
   - `*` 单级通配符
   - `**` 多级通配符

3. **构建扫描列表** - 合并基础包和业务包，去除重复项

4. **创建扫描器** - 使用 `ClassPathBeanDefinitionScanner` 创建扫描器

5. **设置过滤器** - 为 `@Component`、`@Configuration` 和自定义注解添加过滤器（v1.1.0+）

6. **设置排除过滤器**（v1.1.0+）- 为指定的包和类添加排除过滤器

7. **执行扫描** - 扫描所有配置的包路径

8. **注册组件** - 将扫描到的组件注册到 Spring 容器
